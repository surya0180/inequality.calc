...
const Results = () => {
    ...
    if (isValid) {
      ...
      if (comp === ">") {
        setResults({
          ...
          description: [
            ...
            `Hence the expression ${lhs} ${comp} ${rhs} evaluates to ${
              lhs > rhs
            }`,
          ],
        });
      ...
      if (values.length === 0) {
        ...
        setResults({
          status: "failure",
          message:
            "Expression evaluation failed! Please check the following possible reasons",
          description: [
            "Make sure that no two letters or no two operators are placed side by side",
            "Make sure that the comparator does exist in the expression and doesn't come at the end of expression",
            "Make sure that all integer fields are filled correctly",
  ...
  return (
    <div className={"container " + styles.results}>
      ...
      {results.status !== "" && (
        <div className={styles.evalRes}>
          <div
            className={
              results.status === "success" ? styles.success : styles.failure
            }
          >
            {results.status === "success" ? <Check /> : <Clear />}{" "}
            {results.message}
          </div>
