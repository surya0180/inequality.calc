...
const Entity = (props) => {
    ...
    const changeHandler = (event) => {
        ...
        dispatch(valueActions.setInteger({index: idx, value: event.target.value}))
    }
...
const LetterCard = (props) => {
    const dispatch = useDispatch()
    return <Draggable draggableId={props.id.toString()} index={props.index}>
        {(provided) => (
            <div className={'shadow ' + styles.card + " " + styles[props.type] + " " + styles[props.margin]}
                {...provided.draggableProps}
                {...provided.dragHandleProps}
                ref={provided.innerRef}
            >
                {props.action === 'drop' && <IconButton className={styles.closebtn}
                    onClick={() => {
                ...
                <Entity id={props.id} type={props.type} action={props.action} letter={props.letter} />
            </div>
        )}
    </Draggable>;
};
