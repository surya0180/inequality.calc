...
const valueSlice = createSlice({
    ...
    reducers: {
        ...
        removeDrop(state, action) {
            const obj = state.drops.filter(item => item.type === 'letter' ? item.id === parseInt(action.payload) : item.id === action.payload)
            state.drops = state.drops.filter(item => item.type === 'letter' ? item.id !== parseInt(action.payload) : item.id !== action.payload)
            if (obj[0].type === 'letter') {
                state.drags.push(obj[0])
                state.drags.sort((a, b) => a.id - b.id)
            }
        ...
        shuffleDrops(state, action) {
            ...
            const des = parseInt(action.payload.d)
            const id = action.payload.id
            const obj = state.drops.filter(item => id.length > 3 ? item.id === id : item.id === parseInt(id))
